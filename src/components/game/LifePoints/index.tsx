...
export const LifePoints = ({ target, color, align = 'left' }: LifePointsProps) => {
  ...
  return (
    <div className={`relative flex flex-col ${align === 'right' ? 'items-end' : 'items-start'}`}>
      ...
      <div className={`bg-black/80 border-2 p-4 rounded-xl backdrop-blur-xl min-w-[200px] transition-all duration-300
        ${isRedTheme ? 'border-red-600/50 shadow-[0_0_20px_rgba(220,38,38,0.2)]' : 'border-blue-600/50 shadow-[0_0_20px_rgba(37,99,235,0.2)]'}`}>
        
        <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${isRedTheme ? 'text-red-400' : 'text-blue-400'}`}>
          {name}
        </p>
        ...
        <div className="flex items-baseline gap-2">
          ...
          <motion.span 
            className="text-5xl font-black italic text-white tabular-nums tracking-tighter"
          >
            {displayLP}
          </motion.span>
        </div>
