...
export const PlayerHandContainer = ({ isOpponentPlaying }: any) => {
  const { isHidden, setVisible } = useHandController();
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 flex justify-center pointer-events-none">
      <motion.div
        initial={false}
        animate={{ y: 0, opacity: isOpponentPlaying ? 0.25 : 1 }}
        transition={{ type: "spring", stiffness: 200, damping: 25 }}
        className={isOpponentPlaying ? "pointer-events-none" : "pointer-events-auto"}
      >
        <PlayerHand
          cards={player?.hand}
