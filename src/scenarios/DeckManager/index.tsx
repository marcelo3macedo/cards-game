...
export default function DeckManagerScenario({ onBack }: any) {
    ...
    const [gameDatabase] = useState<Card[]>([
        { id: 1, name: "Mago Negro", atk: 2500, def: 2100, type: "MONSTRO", attribute: "TREVAS", level: 7, description: "O mago definitivo em termos de ataque e defesa." },
        { id: 2, name: "Dragão Branco", atk: 3000, def: 2500, type: "MONSTRO", attribute: "LUZ", level: 8, description: "Este lendário dragão é uma poderosa máquina de destruição." },
        { id: 3, name: "Buraco Negro", atk: 0, def: 0, type: "MAGICA", attribute: "FEITIÇO", level: 0, description: "Destrua todos os monstros no campo." },
        { id: 4, name: "Gárgula de Gelo", atk: 2200, def: 1800, type: "MONSTRO", attribute: "ÁGUA", level: 6, description: "Um guardião das cavernas geladas." },
        { id: 5, name: "Força Espelho", atk: 0, def: 0, type: "ARMADILHA", attribute: "ARMADILHA", level: 0, description: "Quando um monstro do oponente declarar um ataque: destrua todos os monstros em posição de ataque do oponente." },
    ]);
    ...
    const addToDeck = (card: Card) => {
        ...
        const updatedDecks = decks.map(d => d.id === activeDeckId ? { ...d, cards: [...d.cards, { ...card, instanceId: Math.random() } as any] } : d);
        setDecks(updatedDecks);
    };

    const removeFromDeck = (instanceId: number) => {
        const updatedDecks = decks.map(d => d.id === activeDeckId ? { ...d, cards: d.cards.filter((c: any) => c.instanceId !== instanceId) } : d);
        setDecks(updatedDecks);
    };
        ...
        <div className="h-screen w-screen bg-zinc-950 flex flex-col text-white overflow-hidden font-sans">
            ...
            <header className="h-16 border-b border-white/5 bg-zinc-900/50 backdrop-blur-xl flex items-center justify-between px-6 z-10">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="font-black italic tracking-widest text-xl bg-gradient-to-r from-blue-400 to-indigo-500 bg-clip-text text-transparent">DECK BUILDER</h1>
                </div>
                
                <div className="flex items-center gap-6">
                    <div className="flex flex-col items-end">
                        <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Capacidade</span>
                        <span className={`text-sm font-black ${activeDeck.cards.length === 40 ? "text-green-500" : "text-yellow-500"}`}>
                            {activeDeck.cards.length} / 40
                        </span>
                    </div>
                    <button 
                        onClick={() => { if(activeDeck.cards.length === 40) alert("Deck Salvo!"); }}
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <section className="flex-[1.4] flex flex-col border-r border-white/5 bg-zinc-900/20">
                    <div className="p-4 bg-black/20 space-y-4">
                        ...
                        <div className="flex gap-2">
                            <select onChange={(e) => setFilterType(e.target.value)} className="flex-1 bg-zinc-800 border border-white/10 rounded-lg p-2 text-[10px] font-bold uppercase outline-none focus:border-blue-500">
                                <option value="TODOS">Todos</option>
                                <option value="MONSTRO">Monstros</option>
                                <option value="MAGICA">Mágicas</option>
                                <option value="ARMADILHA">Armadilhas</option>
                            </select>
                            <button onClick={() => setSortBy(sortBy === "NAME" ? "ATK" : "NAME")} className="px-4 bg-zinc-800 border border-white/10 rounded-lg text-[10px] font-bold uppercase flex items-center gap-2 hover:bg-zinc-700">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-2">
                        {filteredCollection.map((card) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`w-12 h-16 rounded border border-white/10 flex items-center justify-center ${card.type === 'MAGICA' ? 'bg-emerald-950/30' : card.type === 'ARMADILHA' ? 'bg-pink-950/30' : 'bg-orange-950/30'}`}>
                                    ...
                                    <div className="flex-1 ml-4">
                                        ...
                                        <div className="flex gap-2 items-center">
                                            <span className={`text-[8px] px-1.5 py-0.5 rounded border font-bold ${getTypeColor(card.type)}`}>{card.type}</span>
                                            {card.type === "MONSTRO" && (
                                                <span className="text-[9px] font-bold text-zinc-500 uppercase">ATK {card.atk} / DEF {card.def}</span>
                                            )}
                                        </div>
                                    </div>
                ...
                <section className="flex-1 flex flex-col bg-black/40 backdrop-blur-sm">
                    <div className="p-4 border-b border-white/5 flex justify-between items-center bg-zinc-900/30">
                        <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 italic">Composição do Deck</h2>
                        <span className="text-[10px] font-black bg-blue-600/20 text-blue-400 border border-blue-400/20 px-3 py-1 rounded-full">{activeDeck.cards.length} / 40</span>
                    </div>
                        ...
                        {activeDeck.cards.map((card: any) => (
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <div className={`w-1 h-6 rounded-full ${card.type === 'MONSTRO' ? 'bg-orange-500' : card.type === 'MAGICA' ? 'bg-emerald-500' : 'bg-pink-500'}`} />
                                    <div>
                                        <div className="text-[10px] font-black uppercase italic leading-none mb-1">{card.name}</div>
                                        {card.type === "MONSTRO" ? (
                                            <div className="text-[8px] font-bold text-zinc-500 uppercase">⚔️ {card.atk} / 🛡️ {card.def}</div>
                                        ) : (
                                            <div className="text-[8px] font-bold text-zinc-500 uppercase">{card.type}</div>
                                        )}
                                    </div>
                                </div>
                                <button 
                                    onClick={(e) => { e.stopPropagation(); removeFromDeck(card.instanceId); }}
            ...
            <AnimatePresence>
                {viewingCard && (
                    ...
                    >
                        ...
                        >
                            {/* Card Visual Top */}
                            <div className={`h-64 relative flex items-center justify-center ${viewingCard.type === 'MAGICA' ? 'bg-emerald-900/20' : viewingCard.type === 'ARMADILHA' ? 'bg-pink-900/20' : 'bg-orange-900/20'}`}>
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-zinc-900 to-transparent">
                                    <h2 className="text-2xl font-black uppercase italic leading-none">{viewingCard.name}</h2>
                                    <span className={`text-[10px] font-bold uppercase ${getTypeColor(viewingCard.type)} px-2 py-0.5 rounded border inline-block mt-2`}>
                                        {viewingCard.type}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="p-6 space-y-6">
                                <p className="text-zinc-400 text-sm italic leading-relaxed">"{viewingCard.description}"</p>
