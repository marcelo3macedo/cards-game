...
export function PackageCard({ pkg, userLevel, userCoins, onClick }: PackageCardProps) {
  ...
  if (isLocked) {
    return (
      <div className="relative bg-zinc-900/40 border border-white/5 rounded-2xl p-6 opacity-50 grayscale cursor-not-allowed">
  ...
  return (
    <button
      ...
      disabled={!canAfford}
      className={`
        w-full text-left relative bg-zinc-900/60 border rounded-2xl p-6 transition-all
        ${canAfford
...
function PackageCardContent({ pkg, canAfford }: { pkg: StorePackageItem; canAfford: boolean }) {
  return (
    <>
      ...
      <h3 className="text-white font-black text-lg uppercase italic leading-tight mb-1">
        {pkg.name}
      </h3>
      <p className="text-zinc-500 text-xs leading-relaxed mb-5">{pkg.description}</p>
