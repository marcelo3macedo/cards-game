...
import { Trophy, Skull, ArrowRight, Home } from 'lucide-react';
...
export const BattleResultScenario: React.FC<BattleResultProps> = ({
  status,
  opponentName,
  opponentImage,
  opponentMessage,
  ...
  return (
    <div className="h-screen w-screen bg-zinc-950 flex items-center justify-center overflow-hidden relative font-sans text-white">
      {/* Background dinâmico baseado no resultado */}
      <div className={`absolute inset-0 opacity-20 ${isVictory ? 'bg-blue-600' : 'bg-red-900'} blur-[120px]`} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center w-full bg-black/40 backdrop-blur-md border border-white/10 p-10 rounded-3xl shadow-2xl">
          ...
          <div className="flex flex-col items-center gap-4 text-center">
            <div className="relative">
              <div className={`absolute inset-0 rounded-full blur-2xl ${isVictory ? 'bg-blue-500/20' : 'bg-red-500/20'}`} />
              <img 
                src={opponentImage} 
                alt={opponentName} 
            ...
            <p className="text-zinc-400 italic text-lg leading-relaxed mt-4">
              "{opponentMessage}"
            </p>
          </div>

          {/* Lado das Estatísticas / Ações */}
          <div className="flex flex-col items-center md:items-start gap-8">
            <div className="space-y-2">
              <span className="text-zinc-500 uppercase tracking-widest text-sm font-bold">Avaliação do Duelo</span>
